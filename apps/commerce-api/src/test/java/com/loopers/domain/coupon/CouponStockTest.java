...
class CouponStockTest {
    ...
    class Create {
        ...
        void throwException_withInvalidQuantity(Integer quantity) {
            ...
            // when & then
            assertThatException()
                    .isThrownBy(() -> CouponStock.builder()
                            .quantity(quantity)
                            .couponId(couponId)
                            .build()
                    )
        ...
        @DisplayName("쿠폰 아이디가 유효하지 않으면, BusinessException이 발생한다.")
        @NullSource
        @ParameterizedTest
        void throwException_withInvalidProductOptionId(Long couponId) {
            // given
            Integer quantity = 10;
        ...
        @DisplayName("유효한 값이면, 쿠폰 재고를 생성한다.")
        @CsvSource(textBlock = """
                0   | 1
                100 | 2
                999 | 100
                """, delimiter = '|')
        @ParameterizedTest
        void createNewStock_withValidValues(Integer quantity, Long couponId) {
            // when
            ...
            assertThat(stock.getCouponId()).isEqualTo(couponId);
        }
    ...
    class Add {
        ...
        void throwException_whenAmountIsZeroOrNegative(int amount) {
            // given
            CouponStock stock = Instancio.of(CouponStock.class)
                    .set(field(CouponStock::getQuantity), 100)
                    .create();
        ...
        @DisplayName("유효한 증가량이면, 쿠폰 재고 수량을 증가시킨다.")
        @CsvSource(textBlock = """
                100 | 10  | 110
                50  | 50  | 100
                0   | 1   | 1
        ...
        void addQuantity_withValidAmount(int initialQuantity, int amountToAdd, int expectedQuantity) {
            // given
            CouponStock stock = Instancio.of(CouponStock.class)
                    .set(field(CouponStock::getQuantity), initialQuantity)
                    .create();
