...
public class LatestProductCacheManager implements SortedProductCacheManager {

    private final RedisTemplate<String, String> redisTemplate;

    @Override
    public boolean supports(ProductSearchSortType sortType) {
        return sortType == ProductSearchSortType.LATEST;
    }
    ...
    public Page<Long> getProductIds(
        ...
        List<Long> productIds = Objects.requireNonNullElseGet(members, Set::<String>of)
                .stream()
                .map(LatestProductCacheManager::unmemberify)
                .toList();
    ...
    public void saveProduct(Product product) {
        ...
        zo.add(key, member, product.getCreatedAt().toInstant().toEpochMilli());
    ...
    private static String buildKey(@Nullable Long brandId, String version) {
        if (brandId == null) {
            return KEY_PREFIX + version + ":sort:latest";
        } else {
            return KEY_PREFIX + version + ":brand:" + brandId + ":sort:latest";
        }
    }
